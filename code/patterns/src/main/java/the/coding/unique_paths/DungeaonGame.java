...
/**
 * Thought process
 * to find the path in the matrix with minimum sum. return minimum sum. NOOOO
 * ummm.
 * the knight should have power to survive the greatest daemon on the path, which is defined by the most negative value.
 * tough!!
 */
/**
 * Improvements
 * 2d matrix problem. backtracking or Dp.
 * breakdown into subproblems. THINK.
 * 
 * We could start from the destination and deduce backwards the minimal health point that the knight would need at each step along the way.
 * 
 */
public class DungeaonGame {
